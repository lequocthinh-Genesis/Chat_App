<?php
    class UserController{
        ...
        public function getListFriends(){
            $sql = "SELECT * FROM users WHERE NOT unique_id = {$_SESSION['unique_id']} ORDER BY fname ASC";
            ...
            if(mysqli_num_rows($query) == 0){
                ...
                while($row = mysqli_fetch_assoc($query)){
                    $output .= '<a class="friend" href="chat.php?user_id='.$row['unique_id'].'">
                                            <div class="content">
        ...
        public function getListUsers(){
            ...
            if(mysqli_num_rows($query) == 0){
                ...
                while($row = mysqli_fetch_assoc($query)){
                    $output .= '<a class="friend" href="admin_setting_users.php?user_id='.$row['unique_id'].'">
                                            <div class="content">
                                                <img src="api/images/'.$row['img'].'"/>
                                                <div class="details">
                                                    <span>'.$row['lname'].' '.$row['fname'].'</span>
                                                ...
                                                <button class="admin_setting_users" name="'.$row['unique_id'].'">
                                                    Xem chi tiết
                                                </button>
